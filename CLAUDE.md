...
- Because the File Explorer recreates row DOM on collapse/expand and lazy render (which drops the class), the plugin runs a `MutationObserver` (in `main.ts`, `syncExplorerObserver`/`queueExplorerRefresh`) scoped to `.nav-files-container`, re-applying the class on a `requestAnimationFrame` after childList mutations. The observer watches `childList`/`subtree` only — toggling a class is an attribute change, so re-applying cannot self-trigger the observer. It is re-acquired on `layout-change` (the explorer leaf can be recreated) and disconnected in `onunload`, which also strips the class from all rows.
- DOM access uses `activeDocument` (popout-window compatibility, `obsidianmd/prefer-active-doc`); timer calls use `window.*` (`obsidianmd/prefer-window-timers`).
